...
```{r}
parsed <- fromJSON(rawToChar(friedrice_info$content)) 
friedrice_data <- as_tibble(parsed$data) 
friedrice_data
```
...
```
```{r}
myData <- GET("https://api.nal.usda.gov/fdc/v1/foods/search?api_key=gRRFVxFCr5iNxkKTElVjMgYyvsonZELMmQP7feiJ&query=Cheddar%20Cheese")
```
